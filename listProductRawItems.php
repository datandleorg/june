                            ...
                            <div class="table-responsive">
                                <table id="example1" class="table table-bordered table-hover display">
                                    ...
                                    <tbody>
                                        ...
                                        <?php												
                                        include("database/db_conection.php");//make connection here
										// $sql = "select r.handler as rawhandler, r.*,p.*,c.orgname from rawitemaster r,salesitemaster2 p, comprofile c "; 
										// $sql.= " where p.itemcode=r.proditemcode and c.orgid=r.orgid order by r.id asc";

										if($orgType=="self"){
											$sql = "SELECT r.handler as rawhandler, r.*,s.*,c.orgname from rawitemaster r,salesitemaster2 s, comprofile c where s.itemcode=r.proditemcode ";
											$sql.= $orgidUrl!==''  ? " and s.orgid='".$orgidUrl."' and s.orgid=c.orgid " : " and s.orgid=c.orgid";
										    $sql.= " ORDER BY r.id ASC ";

										}else if($orgType=="outsourced"){
											$sql = "SELECT r.handler as rawhandler, r.*,s.*,cust.custid as orgid,cust.custname as orgname from rawitemaster r,salesitemaster2 s, customerprofile cust where s.itemcode=r.proditemcode ";
											$sql.= $orgidUrl!==''  ? " and s.custid='".$orgidUrl."' and s.custid=cust.custid " : " and s.custid=cust.custid";
											$sql.= " ORDER BY r.id ASC ";

										}else{
											$sql = "select r.entrytype, r.id as id,r.rw_code, c.orgid as orgid,c.orgname as orgname,r.proditemcode,s.itemname,r.handler as rawhandler  from rawitemaster r,salesitemaster2 s, comprofile c
											 where s.orgid=c.orgid and s.itemcode=r.proditemcode 
											union 
											select r.entrytype,r.id as id,r.rw_code, cus.custid as orgid,cus.custname as orgname,r.proditemcode,s.itemname,r.handler as rawhandler from rawitemaster r,salesitemaster2 s, customerprofile cus where s.custid=cus.custid
											and s.itemcode=r.proditemcode ";
											$sql.= " ORDER BY id ASC ";
