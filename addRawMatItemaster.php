							...
							<div class="card-body p-4">
								...
								<form id="add_rawmaterial_form" autocomplete="off" enctype="multipart/form-data" method="post">
                                    ...
                                    <div class="form-row">
                                        ...
                                        <select class="form-control form-control-sm select2" name="entrytype"
                                         id="entrytype"
                                    ...
                                    <div class="form-group col-md-12">
                                        ...
                                        ?> 
                                         <?php if(isset($_GET['entrytype'])&&$_GET['entrytype']!="outsourced")
                                         {
                                            echo  'id="orgid"';
                                            echo  ' ';
                                            echo  'name="orgid"';
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            include("database/db_conection.php");//make connection here
                                             ...
                                             $sql = "SELECT itemcode,concat(itemcode,'-',itemname) as itemname FROM purchaseitemaster p";            
                                             if($entryType=="self"){
                                                $sql.= $orgidUrl!='' ? ", comprofile comp WHERE comp.orgid='".$orgidUrl."' AND comp.orgid=p.orgid " : "  ";
                                             }else{
                                                $sql.= $orgidUrl!='' ? " , customerprofile cust WHERE cust.custid='".$orgidUrl."' AND cust.custid=p.custid " : "  ";
                                             }      
                                             // $sql;
                                             $exe = mysqli_query($dbcon,$sql);
                                            while ($row = $exe->fetch_assoc()){	
...
<script>
    ...
    var error = false;
        ...
        function setUom(table,ele){
           ...
           $(ele).closest('tr').find('#uom').val(itemData.uom);
